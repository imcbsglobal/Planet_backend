...
def api_collection_add(request):
    ...
    if request.user.is_authenticated:
        ...
        username = (request.data.get('created_by_name') or request.data.get('created_by') or '').strip()
        if username:
...
def api_collection_detail(request, pk):
    ...
    serializer = CollectionSerializer(obj, data=request.data, partial=partial, context={'request': request})
    if serializer.is_valid():
        serializer.save()
        return Response(serializer.data)
    return Response({'message': 'Validation failed.', 'errors': serializer.errors},
